...
def test_apply_writes_tags_art_and_backup(mmf, tmp_path):
    ...
    mmf.run_apply(Namespace(
        ...
        no_backup=False, tmp_dir=str(tmp_path / "tmp"),
    ))
    ...
    assert len(list((tmp_path / "tmp" / "backups").iterdir())) == 1
...
def test_convert_keeps_tags(mmf, tmp_path, capsys):
    ...
    mmf.run_convert(Namespace(
        root=str(root), from_ext="FLAC", to_ext="mp3", no_backup=True,
    ))
...
def test_convert_same_format_is_noop(mmf, tmp_path, capsys):
    mmf.run_convert(Namespace(
        root=str(tmp_path), from_ext="mp3", to_ext=".MP3", no_backup=True,
    ))
...
def test_normalize_keeps_tags(mmf, tmp_path, capsys):
    ...
    mmf.set_tags(mmf.load_track(song), TAGS)
    mmf.run_normalize(Namespace(root=str(root), lufs=-16.0, no_backup=True))
